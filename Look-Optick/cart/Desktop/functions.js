...
var FP_LP_DONATIONS_TRUST = {
    ...
    addTitle: function() {
        $('.fp_progressbar').after('<div layout="column" layout-align="start center" class="fp_progressbar_container lo-cart-wrapper fullwidth"><h1 class="fp_page-title"> SHOPPING CART </h1></div>')
    },
    virtualMirror: function() {

        $('.cart__product-image-wrapper').append($('.fyl__section__btn.vm__trigger'))
        ...
        setInterval(function() {
            if (!$('.fyl__section__btn.vm__trigger').is(":visible")) {
                $('body').append('<script defer src="//cdn.shopify.com/s/files/1/0797/7215/t/312/assets/virtual_mirror.bundle.js?v=8574063332416305004" type="text/javascript"></script>')
                $('.fyl__section__btn.vm__trigger').show()
            }
        }, 2000)
        ...
        $(document).on('click', '.cart__product-image-wrapper .fyl__section__btn.vm__trigger', function() {
            ...
            var clickedProductName = $(this).parent('.cart__product-image-wrapper').parent('.cart__product-wrapper').children('.cart__product-info-wrapper').children('div').children('a.cart__product-title').text().split(' ');
            ...
            if (counter == 0) {
                var firstTimeSelect = setInterval(function() {
                    for (var i = 0; i < clickedProductName.length; i++) {
                        ...
                        var a = $(that).parent('.cart__product-image-wrapper').parent('.cart__product-wrapper').children('.cart__product-info-wrapper').children('div').children('a.cart__product-title').text().split(' ');
    ...
    changeButtonsPosition: function() {
        // change x button and price position 
        $('.cart__product-info-wrapper').each(function(item) {
            $(this).children('div.fullwidth:first-child').append($(this).children('div.full-width:last-child').children('.cart__product-price'))

            $(this).children('div.full-width:last-child').append($(this).children('div.fullwidth:first-child').children('.cart__remove'))
    ...
    editProductModal: function() {
        ...
        var determineModal = '<div class="fp_modal_container fp_modal_container--edit"><div class="fp_modal_klaviyo-bis-close fadein"><div class="w-100" id="klaviyo-bis-modal"><div class="fp_container"><div class="fp_appended"></div><button type="button" class="fp_close klaviyo-bis-close" data-dismiss="modal"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1L15 15" stroke="white" stroke-width="2" stroke-miterlimit="1.5" stroke-linejoin="round"/><path d="M15 1L1 15" stroke="white" stroke-width="2" stroke-miterlimit="1.5" stroke-linejoin="round"/></svg></button></div></div></div></div>';
    ...
    fetchData: function(url, img) {
        ...
        jQuery.ajax({
            ...
            $('.fp_modal_container--edit .fp_container .fp_appended').append('<div class="fp_appended_left"><img id="ProductPhotoImg" src="' + src + '"></div><div class="fp_appended_right"></div>')

            // append title and price
            var titleAndPrice = a.find('.product-single__header-wrapper')
            $('.fp_modal_container--edit .fp_container .fp_appended_right').append(titleAndPrice)
            $('.fp_appended .product-single__classification--wrapper').hide();

            $('.fp_modal_container--edit .fp_container .fp_appended_right').append(a.find('.variant-options'))
            ...
            $('.fp_appended_left').append('<button id="AddToCart" disabled type="submit" name="add" fullwidth=""  class="lo-btn AddToCart"><span id="AddToCartText">UPDATE ITEM</span></button>')
    ...
    belowSummaryImages: function() {
        ...
        $('.cart__checkout-list').append(list);
    ...
    reviews: function() {
        ...
        var reviewSample = '<div class="fp_reviews_container"></div>'


        var reviewsText = '<div class="fp_reviews lo-cart-wrapper lo-section fullwidth">' + title + reviewSample + ' </div>'
        ...
        function makeReviews(reviews, titles) {
            for (var j = 0; j < reviews.length; j++) {
                $('.fp_reviews_container').append('<div class="fp_review"> <div class="fp_review_head"><span class="title">' + titles[j] + '</span> <span class="stars_container">' + stars + ' <span>5.0 </span></span></div> <div class="reviewDescription">' + reviews[j] + ' </div> </div>')
            }
            // var i = 0;
